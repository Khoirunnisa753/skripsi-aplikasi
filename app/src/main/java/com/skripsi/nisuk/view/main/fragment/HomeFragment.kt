...
class HomeFragment : Fragment(), OnMapReadyCallback {
    ...
    override fun onCreateView(
        ...
        if (usernameMonitoring != ""){
            ...
            binding.etUsername.setText(usernameMonitoring)
            fetchLocationsFromFirebase(usernameMonitoring.toString())
        }
        binding.addUsername.setOnClickListener {
            binding.sendUsername.visibility = View.VISIBLE
            binding.etUsername.visibility = View.VISIBLE
            binding.cancel.visibility = View.VISIBLE
            binding.addUsername.visibility = View.GONE
        ...
        binding.cancel.setOnClickListener {
            ...
            binding.sendUsername.visibility = View.GONE
            binding.addUsername.visibility = View.VISIBLE
            binding.cancel.visibility = View.GONE
        }
        binding.sendUsername.setOnClickListener {
            val username = binding.etUsername.text.toString()
            val refUsername = database.getReference("akun_tunanetra")

            refUsername.child(username).get().addOnSuccessListener { snapshot ->
                if (snapshot.exists()) {
                    // Username  ada
                    binding.etUsername.visibility = View.GONE
                    binding.sendUsername.visibility = View.GONE
                    ...
                    SharedPreferencesHelper.saveUsername(username, requireContext())
                    binding.map.visibility = View.VISIBLE
                    binding.infoLayout.visibility = View.VISIBLE
                    fetchLocationsFromFirebase(username)
                    Toast.makeText(requireContext(), "Username disimpan", Toast.LENGTH_SHORT).show()
                } else {
                    Toast.makeText(requireContext(), "Username tidak ditemukan", Toast.LENGTH_SHORT).show()

                }
            }.addOnFailureListener {
                Toast.makeText(requireContext(), "Gagal cek database: ${it.message}", Toast.LENGTH_SHORT).show()
            }
        }
    ...
    private fun getFormattedDate(locale: Locale): String {
        val dateFormat = SimpleDateFormat("dd MMMM yyyy", locale)
        return dateFormat.format(Date())
    }

    // Ambil data lokasi dari Firebase Realtime Database
    private fun fetchLocationsFromFirebase(username: String) {
        val locale = Locale("id", "ID")
        val date = Date()
        val sdf = SimpleDateFormat("dd MMM yyyy", locale)
        val indoDate = sdf.format(date)
        val locale1 = Locale.ENGLISH
        val date1 = Date()
        val sdf1 = SimpleDateFormat("dd MMM yyyy", locale1)
        val engDate = sdf1.format(date1)        // Coba ambil data dengan format Indonesia dulu
        ...
        trackingRef.addListenerForSingleValueEvent(object : ValueEventListener {
            override fun onDataChange(snapshot: DataSnapshot) {
                if (snapshot.exists()) {
                    ...
                    // Jika tidak ada data di path bahasa Indonesia, coba versi bahasa Inggris
                    ...
                    fallbackRef.addListenerForSingleValueEvent(object : ValueEventListener {
                        override fun onDataChange(fallbackSnapshot: DataSnapshot) {
                            if (fallbackSnapshot.exists()) {
                                ...
                                Log.w("HomeFragment", "Tidak ada data lokasi pada path bahasa Indonesia maupun Inggris.")
                            }
                        }

                        override fun onCancelled(error: DatabaseError) {
                            Toast.makeText(requireContext(), "Gagal ambil data: ${error.message}", Toast.LENGTH_SHORT).show()
                            Log.e("HomeFragment", "Gagal ambil data (fallback): ${error.message}")
                        }
                    })
                }
            }
    ...
    // Jangan lupa untuk menambahkan lifecycle method MapView
